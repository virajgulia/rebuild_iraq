...
class PasswordField extends Component {
  ...
  validatePasswordStrong = value => {
    ...
    const regexp = /^(?=(?:.*[A-Z]){1,})(?=(?:.*[a-z]){1,})(?=(?:.*\d){1,})(?=(?:.*[!@#$%^&*()\-_=+{};:,<.>]){1,})(?!.*(.)\1{2})([A-Za-z0-9!@#$%^&*()\-_=+{};:,<.>]{7,20})$/;
    ...
    // ensure password contains at least one capital letter, number and special character
    if (!regexp.test(value)) throw new Error("Password is weak");
  };
      ...
      <Fragment>
        <div className="position-relative">
          ...
          <FormField type="password" validator={this.validatePasswordStrong} onStateChanged={this.stateChanged} {...restProps}>
            <span className="d-block form-hint">To ensure account safety, password must be more than 7 characters.</span>
