...
class FormField extends Component {
  ...
  hasChanged = e => {
    ...
    const { label, required = false, validator = f => f, onStateChanged = f => f, setError = f => f } = this.props;
  ...
  render() {
    ...
    const { type, label, fieldId, placeholder, children, role } = this.props;
    ...
    return (
      <Fragment>
        <div className={`form-group ${role === 'login' ? '' : 'px-3 pb-2'}`}>
